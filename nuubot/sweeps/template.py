    ...
    for index, (data_set, signaler_set, executor_set, risk) in enumerate(product(data_sets, signaler_sets, executor_sets, risk_values), start=1):
        ...
        config_row = {
            "sweeprun": {
                ...
                "investment_usdc": config.sweep.get("investment_usdc", 10000.0),
                "trade_use": config.sweep.get("trade_use", "pct"),
                "trade_amount": config.sweep.get("trade_amount", 100.0),
                "trade_pct": config.sweep.get("trade_pct", 2.0),
                "meta": meta,
            },
