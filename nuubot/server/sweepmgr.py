...
class SweepManager:
    ...
    def list(self) -> dict[str, Any]:
        db_dir = Path(self.nuubot.config.workspace.root) / self.nuubot.config.paths.db_dir
        rows: list[dict[str, Any]] = []
        for path in sorted(db_dir.glob("sweep_*.db"), key=_sweep_id_from_path):
            sweep_id = _sweep_id_from_path(path)
            with self.nuubot.datastore.session(path) as session:
    ...
    def run(self, sweep_id: int) -> dict[str, Any]:
        return Sweep(self.nuubot, sweep_id, self.finalizers).run()
